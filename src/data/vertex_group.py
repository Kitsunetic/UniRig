...
class VertexGroupVoxelSkin(VertexGroup):
    ...
    def get_vertex_group(self, asset: Asset) -> Dict[str, ndarray]:
        ...
        skin = np.nan_to_num(skin, nan=0.0, posinf=0.0, neginf=0.0)
        return {
            "voxel_skin": skin,
        }
...
def get_geodesic_distance(
    ...
    # (J, 3)
    offset = tails - joints
    inv = (1.0 / (offset * offset + eps).sum(axis=-1))[np.newaxis, ...]
    # head
    ...
    # (N, J)
    scale0 = (np.clip(c0, 0.0, 1.0) + eps) / (np.clip(c0, 0.0, 1.0) + np.clip(c1, 0.0, 1.0) + eps * 2)
    scale1 = -scale0 + 1
    # (N, J, 3)
...
def voxelization(
    ...
    if backend == "pyrender":
        ...
        camera_poses["+y"] = np.array(
            ...
            dtype=np.float32,
        )  # look at +y (because model is looking at -y)(front to back)
        camera_poses["-y"] = np.array(
            [
                [1, 0, 0, 0],
                [0, 0, 1, eye_dis],
        ...
        grid_coords = (
            np.stack((i.ravel(), j.ravel(), grid - 1 - k.ravel()), axis=1, dtype=np.float32) * 2 / grid - 1.0 + 1.0 / grid
        )  # every position is in the middle of the grid
        depths = {}
        for cam_node in camera_nodes:
            # a = time.time()
            scene.main_camera_node = cam_node
            name = cam_node.name
            proj_depth = renderer.render(
                scene, flags=pyrender.constants.RenderFlags.DEPTH_ONLY | pyrender.constants.RenderFlags.OFFSCREEN
            )
    ...
    for component_id in range(tot):
        ...
        if n_component >= k:
            ...
            sampled_indices = np.concatenate(
                [np.random.permutation(n_component), np.random.randint(0, n_component, k - n_component)]
            )
