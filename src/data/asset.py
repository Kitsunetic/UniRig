...
class Asset(Exporter):
    ...
    def get_matrix(self, matrix_basis: ndarray, matrix_local: Union[ndarray, None] = None):
        ...
        if matrix_local is None:
            ...
            if matrix_local is None:
                ...
                matrix_local[:, 0, 0] = 1.0
                matrix_local[:, 1, 1] = 1.0
                matrix_local[:, 2, 2] = 1.0
                matrix_local[:, 3, 3] = 1.0
        ...
        for i in range(self.J):
            if i == 0:
                ...
                matrix[i] = matrix_parent @ (np.linalg.inv(matrix_local_parent) @ matrix_local[i]) @ matrix_basis[i]
        return matrix
    ...
    def apply_matrix_basis(self, matrix_basis: ndarray):
        ...
        vertices = linear_blend_skinning(self.vertices, matrix_local, matrix, self.skin, pad=1, value=1.0)
        # update matrix_local
        ...
        if self.tails is not None:
            ...
            self.tails = linear_blend_skinning(self.tails, matrix_local, matrix, t_skin, pad=1, value=1.0)
