...
from ..tokenizer.spec import DetokenizeOutput, TokenizerSpec
...
class UniRigAR(ModelSpec):
        ...
        res = [
            {
                ...
                "attention_mask": np.pad(
                    torch.ones(b.tokens.shape[0]), ((0, max_length - b.tokens.shape[0])), "constant", constant_values=0.0
                ),
            }
            for b in batch
        ]
    ...
    def __init__(self, llm, mesh_encoder, **kwargs):
        ...
        if isinstance(self.mesh_encoder, MAP_MESH_ENCODER.michelangelo) or isinstance(
    ...
    def training_step(self, batch: Dict) -> Dict[str, FloatTensor]:
        ...
        attention_mask = pad(attention_mask, (cond.shape[1], 0, 0, 0), value=1.0)
    ...
    def generate(
        ...
        **kwargs,
        ...
        start_embed = self.transformer.get_input_embeddings()(start_tokens.unsqueeze(0)).to(dtype=self.transformer.dtype)
