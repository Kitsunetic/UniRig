    ...
    def _process_fn(self, batch: List[ModelInput]) -> List[Dict]:
        ...
        for id, b in enumerate(batch):
            ...
            if b.asset.joints is not None:
                n_batch[id]["joints"] = np.pad(
                    b.asset.joints, ((0, max_bones - b.asset.J), (0, 0)), mode="constant", constant_values=0.0
                )
                n_batch[id]["num_bones"] = b.asset.J
            if b.asset.tails is not None:
                n_batch[id]["tails"] = np.pad(
                    b.asset.tails, ((0, max_bones - b.asset.J), (0, 0)), mode="constant", constant_values=0.0
                )
            ...
            if b.asset.matrix_local is not None:
                J = b.asset.J
                matrix_local = np.pad(
                    b.asset.matrix_local, ((0, max_bones - J), (0, 0), (0, 0)), "constant", constant_values=0.0
                )
                # set identity to prevent singular matrix in lbs
                matrix_local[J:, 0, 0] = 1.0
                matrix_local[J:, 1, 1] = 1.0
                matrix_local[J:, 2, 2] = 1.0
                matrix_local[J:, 3, 3] = 1.0
                n_batch[id]["matrix_local"] = matrix_local
            if b.asset.pose_matrix is not None:
                J = b.asset.J
                pose_matrix = np.pad(b.asset.pose_matrix, ((0, max_bones - J), (0, 0), (0, 0)), "constant", constant_values=0.0)
                pose_matrix[J:, 0, 0] = 1.0
                pose_matrix[J:, 1, 1] = 1.0
                pose_matrix[J:, 2, 2] = 1.0
                pose_matrix[J:, 3, 3] = 1.0
