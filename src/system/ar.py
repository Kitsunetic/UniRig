...
from .metrics import B2B, J2B, J2J
...
class ARSystem(L.LightningModule):
    ...
    def on_validation_batch_end(self, outputs, batch, batch_idx, dataloader_idx=0):
        ...
        try:
            if self.validate_cast == "bfloat16":
                with torch.autocast("cuda", dtype=torch.bfloat16):
                    prediction: List[DetokenizeOutput] = self._predict_step(
        ...
        for id, res in enumerate(prediction):
            ...
            self._validation_loss[f"val_{cls}_fail"].append(0.0)
            if self.record_res:
    ...
    def on_validation_epoch_end(self):
        ...
        if tot_j2j == 0:
            self._validation_loss["val_loss_j2j"] = 1.0
        else:
...
class ARWriter(BasePredictionWriter):
    def __init__(self, output_dir: Union[str, None], order_config: Union[OrderConfig, None] = None, **kwargs):
    ...
    def write_on_batch_end(
        self, trainer, pl_module: ARSystem, prediction: List[Dict], batch_indices, batch, batch_idx, dataloader_idx
    ):
        ...
        for id, detokenize_output in enumerate(detokenize_output_list):
            assert isinstance(
                detokenize_output, DetokenizeOutput
            ), f"expect item of the list to be DetokenizeOutput, found: {type(detokenize_output)}"
