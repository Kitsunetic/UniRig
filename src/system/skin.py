    ...
    def forward(self, batch, validate: bool = False) -> Dict[str, Tensor]:
        ...
        if validate:
            ...
            if self.record_res:
                ...
                exporter = Exporter()
                exporter._export_pc(
                    vertices=vertices_gt[indices], path=f"{self.output_path}/{self.current_epoch}/{path}/vertices_gt.obj"
                )
                exporter._export_pc(
                    vertices=vertices_pred[indices], path=f"{self.output_path}/{self.current_epoch}/{path}/vertices_pred.obj"
                )
        else:
            log_loss_dict = {}
            for name in self.loss_config:
...
class SkinWriter(BasePredictionWriter):
    def __init__(self, output_dir: Union[str, None], save_name: str, order_config: Union[OrderConfig, None] = None, **kwargs):
    ...
    def write_on_batch_end(
        self, trainer, pl_module: SkinSystem, prediction: List[Dict], batch_indices, batch, batch_idx, dataloader_idx
    ):
