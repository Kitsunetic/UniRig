...
def J2J(joints_a: FloatTensor, joints_b: FloatTensor, continuous_range: Tuple[float, float]) -> FloatTensor:
    """
    joints_a: (J1, 3) joint
...
def points_to_segments_distance(
    coord: FloatTensor, bones_head: FloatTensor, bones_tail: FloatTensor, keepdims: bool, eps: float = 1e-6
) -> FloatTensor:
    # (J, 3)
    offset = bones_tail - bones_head
    inv = (1.0 / (offset * offset + eps).sum(dim=-1)).unsqueeze(0)
    # head
    ...
    # (N, J)
    scale0 = (c0.clamp(min=0.0, max=1.0) + eps) / (c0.clamp(min=0.0, max=1.0) + c1.clamp(min=0.0, max=1.0) + eps * 2)
    scale1 = -scale0 + 1
    # (N, J, 3)
...
def B2B(bones_a: FloatTensor, bones_b: FloatTensor, continuous_range: Tuple[float, float]) -> FloatTensor:
    """
    bones_a: (J1, 6) (position parent, position)
