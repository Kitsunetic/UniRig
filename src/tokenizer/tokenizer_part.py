...
from .spec import DetokenizeOutput, TokenizeInput, TokenizerConfig, TokenizerSpec, make_skeleton
    ...
    def detokenize(self, ids: ndarray, **kwargs) -> DetokenizeOutput:
        ...
        while i < len(ids):
            if ids[i] < self.num_discrete:
                if is_branch:
                    p_joint = undiscretize(
                        t=ids[i : i + 3], continuous_range=self.continuous_range, num_discrete=self.num_discrete
                    )
                    current_joint = undiscretize(
                        t=ids[i + 3 : i + 6], continuous_range=self.continuous_range, num_discrete=self.num_discrete
                    )
