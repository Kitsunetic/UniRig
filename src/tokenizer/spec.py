...
from ..data.order import Order, OrderConfig, get_order
...
def make_skeleton(
    ...
    """
    assert (
        convert_leaf_bones_to_tails & extrude_tail_for_leaf
    ) == False, "cannot extrude tail for leaf when convert_leaf_bones_to_tails is True"
    ...
    for i, pid in enumerate(parents):
        ...
        if extrude_tail_for_branch:
            if pid is None:  # root
                ...
                d = bones[i, 3:] + np.array([0.0, 0.0, extrude_scale * av_len])
            else:
                d = bones[i, 3:] - bones[pid, 3:]
                length = np.linalg.norm(d)
                if strict:
